...
<div id="cuerpo" class="cuerpo">
  ...
  <div class="clearfix"></div>
    ...
    <div class="panel-body">
  		<form id="form_act" name="form_act" role="form" class="form-horizontal form-groups-standar validate" method="post">

				<div class="form-group">
          <label class="col-sm-2 control-label"><b class="rojo">*</b> usuario</label>
          <div class="col-sm-5"><input name="usuario" type="text" class="form-control" value="<? echo $row["usuario"]; ?>" data-validate="email" data-message-required="No tiene un formato de e-mail" autofocus></div>
        </div>

				<? if($v1=="si"){ ?>
	      	<div class="form-group">
	          <label class="col-sm-2 control-label">contrase&ntilde;a</label>
	          <div class="col-sm-4"><input name="passw" type="password" class="form-control" placeholder="introducir sólo para cambiar"></div>
	        </div>
				<? } ?>
          ...
          <div class="col-sm-4"><input name="nombre" type="text" class="form-control" value="<? echo $row["nombre"]; ?>" data-validate="required" data-message-required="El campo es obligatorio"></div>
        </div>

				<div class="form-group">
          <label class="col-sm-2 control-label">apellidos</label>
          <div class="col-sm-6"><input name="apellidos" type="text" class="form-control" value="<? echo $row["apellidos"]; ?>"></div>
        </div>
        ...
        <? } ?>
	          ...
	          <div class="col-sm-4"><input name="tlf" type="text" class="form-control" value="<? echo $row["tlf"]; ?>"></div>
	        </div>
				<? } ?>
	          ...
	          <div class="col-sm-4"><input name="nif" type="text" class="form-control" value="<? echo $row["nif"]; ?>"></div>
	        </div>
				<? } ?>
					...
					<div class="form-group">
						...
						while($row_etiquetas = mysqli_fetch_array($rs_etiquetas)){
							...
							echo "<div class='col-sm-3' style='padding-left:0'>";
								$sql_item = "SELECT * FROM registrados_rel_etiquetas WHERE (ref_reg=".$ref." AND ref_etiqueta=".$row_etiquetas["ref"].")";
...
$(document).ready(function(){
    ...
    $.ajax({
			type: "POST",
			dataType: "json",
			url: "data/registrados_act.php?mod=<? echo $mod; ?>&ref=<? echo $ref; ?>",
