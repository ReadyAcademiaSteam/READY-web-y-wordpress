...
while($row_listaidiomas = mysqli_fetch_array($rs_listaidiomas)){
	...
	$sql_esta = "SELECT * FROM contenidos_fotos_info WHERE ref_foto=".$ref." AND ref_idioma=".$row_listaidiomas["ref"];
	$rs_esta = mysqli_query($conn,$sql_esta);
	if(mysqli_num_rows($rs_esta)>0){
		$sql_actid = "UPDATE contenidos_fotos_info SET ";
		$sql_actid.= "nombre='".$_POST[$campo1]."'";
		$sql_actid.= " WHERE ref_foto=".$ref." AND ref_idioma=".$row_listaidiomas["ref"];
		$rs_actid = mysqli_query($conn,$sql_actid);
	}else{
		$sql_ins = "INSERT INTO contenidos_fotos_info (ref_foto,ref_idioma,nombre) VALUES ('".$ref."','".$row_listaidiomas["ref"]."','".$titulo."')";
