    ...
    function upload_course_package_call(){
        if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'upload_course_package_call') || !is_user_logged_in()){
            ...
            if(!empty($course_id) && is_numeric($course_id) && !empty($arr[2]) && !empty($arr[3])){
                $package_meta = array('name'=>$arr[2],'path'=>$arr[1],'file'=>$arr[3],'package_type'=>$_POST['package_type']);
    ...
    function put_json_for_selected_options(){
        ...
        if (count($dirs)>0){
            ...
            if(!empty($dirs)){
                ...
                <label for="package_type_select">'._x('Select package type','','wplms-front-end').'</label><select style="width:100%" class="package_type_select"><option value="">'._x('Select package type','','wplms-front-end').'</option>
                <option value="default">'. __('Default (xAPI or None)','vibe-shortcodes').'</option><option value="2004">'. __('SCORM 2004','vibe-shortcodes').'</option><option value="1.1">'. __('SCORM 1.1, 1.2','vibe-shortcodes').'</option>
                </select><br>
                        ...
                        //$('.stop_vimeo_plupload_upload').addClass('disabled');
                         $.ajax({
                          ...
                          data: { action: 'upload_course_package_call', 
                                  ...
                                  name:file.name,
                                  type:file.type,
                                  package_type:$('body').find('.package_type_select').val(),
