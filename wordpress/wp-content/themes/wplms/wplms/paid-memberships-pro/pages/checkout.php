...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<?php if($pmpro_msg) 
		{
	?>
		<div id="pmpro_message" class="pmpro_message <?php echo vibe_sanitizer($pmpro_msgt,'text')?>"><?php echo vibe_sanitizer($pmpro_msg)?></div>
	<?php
		}
		else
		{
	...
	<?php if($pmpro_review) { ?>
		<p><?php _e('Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'vibe');?></p>
	<?php } ?>
	...
	<thead>
		<tr>
			<th>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg"><a href="<?php echo pmpro_url("levels"); ?>"><?php _ex('change', 'change membership level link', 'vibe');?></a></span><?php } ?><?php _e('Membership Level', 'vibe');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
		<tr>
			<td>				
				<p>
					<?php printf(__('You have selected the <strong>%s</strong> membership level.', 'vibe'), $pmpro_level->name);?>
				</p>
				...
				<div id="pmpro_level_cost">
					<?php if($discount_code && pmpro_checkDiscountCode($discount_code)) { ?>
						<?php printf(__('<p class="pmpro_level_discount_applied">The <strong>%s</strong> code has been applied to your order.</p>', 'vibe'), $discount_code);?>
					<?php } ?>
				...
				<?php if($pmpro_show_discount_code) { ?>
					...
					<?php if($discount_code && !$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><a id="other_discount_code_a" href="#discount_code"><?php _e('Click here to change your discount code', 'vibe');?></a>.</p>
					<?php } elseif(!$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><?php _e('Do you have a discount code?', 'vibe');?> <a id="other_discount_code_a" href="#discount_code"><?php _e('Click here to enter your discount code', 'vibe');?></a>.</p>
					<?php } elseif($pmpro_review && $discount_code) { ?>
						<p><strong><?php _e('Discount Code', 'vibe');?>:</strong> <?php echo vibe_sanitizer($discount_code)?></p>
					<?php } ?>
		...
		<tr id="other_discount_code_tr" style="display: none;">
			<td>
				<div>
					<label for="other_discount_code"><?php _e('Discount Code', 'vibe');?></label>
					...
					<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e('Apply', 'vibe');?>" />					
				</div>				
			</td>
		</tr>
		<?php } ?>
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-msg"><?php _e('Already have an account?', 'vibe');?> <a href="<?php echo wp_login_url(pmpro_url("checkout", "?level=" . $pmpro_level->id))?>"><?php _e('Log in here', 'vibe');?></a>.</span><?php _e('Account Information', 'vibe');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
		<tr>
			<td>
				<div>
					<label for="username"><?php _e('Username', 'vibe');?></label>
				...
				<div>
					<label for="password"><?php _e('Password', 'vibe');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="password2"><?php _e('Confirm Password', 'vibe');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="bconfirmemail"><?php _e('Confirm E-mail Address', 'vibe');?></label>
				...
				<div class="pmpro_hidden">
					<label for="fullname"><?php _e('Full Name', 'vibe');?></label>
					<input id="fullname" name="fullname" type="text" class="input <?php echo pmpro_getClassForField("fullname");?>" size="30" value="" /> <strong><?php _e('LEAVE THIS BLANK', 'vibe');?></strong>
				</div>				
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>                        	                       										
		...
		<p id="pmpro_account_loggedin">
			<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'vibe'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>			
		</p>
	<?php } ?>
	...
	<?php if(pmpro_getOption("gateway", true) == "paypal" && empty($pmpro_review)) { ?>
		...
		<thead>
			<tr>
				<th><?php _e('Choose your Payment Method', 'vibe');?></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>
					<div>
						<input type="radio" name="gateway" value="paypal" <?php if(!$gateway || $gateway == "paypal") { ?>checked="checked"<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Check Out with a Credit Card Here', 'vibe');?></a> &nbsp;
						<input type="radio" name="gateway" value="paypalexpress" <?php if($gateway == "paypalexpress") { ?>checked="checked"<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Check Out with PayPal', 'vibe');?></a> &nbsp;					
					</div>
				</td>
			</tr>
		</tbody>
	...
	<tbody>
		<tr>
			<td>
				...
				?>
					<div>
						<label for="bcity"><?php _e('City', 'vibe');?></label>
					...
					<div>
						<label for="bstate"><?php _e('State', 'vibe');?></label>																
						...
						<?php
							...
							<select name="bstate" class=" <?php echo pmpro_getClassForField("bstate");?>">
								...
								<?php 									
									...
									{ 
								?>
									<option value="<?php echo esc_attr($ab);?>" <?php if($ab == $bstate) { ?>selected="selected"<?php } ?>><?php echo vibe_sanitizer($st);?></option>
								<?php } ?>
							</select>
							<?php
							}
							...
							?>
								<select name="bstate" class=" <?php echo pmpro_getClassForField("bstate");?>">
									...
									<?php 									
										...
										{ 
									?>
										<option value="<?php echo esc_attr($ab);?>" <?php if($ab == $bstate) { ?>selected="selected"<?php } ?>><?php echo vibe_sanitizer($ab);?></option>
									<?php } ?>
								</select>
							<?php
							}
				...
				<div>
					...
					<select name="bcountry" class=" <?php echo pmpro_getClassForField("bcountry");?>">
						<?php
							...
							{								
							?>
							<option value="<?php echo vibe_sanitizer($abbr)?>" <?php if($abbr == $bcountry) { ?>selected="selected"<?php } ?>><?php echo vibe_sanitizer($country)?></option>
							<?php
							}
						?>
					</select>
	...
	<thead>
		<tr>
			<th><span class="pmpro_thead-msg"><?php printf(__('We Accept %s', 'vibe'), $pmpro_accepted_credit_cards_string);?></span><?php _e('Payment Information', 'vibe');?></th>
		</tr>
	</thead>
	<tbody>                    
		<tr valign="top">		
			<td>	
				...
				<div>
					...
					<select id="CardType" <?php if($gateway != "stripe") { ?>name="CardType"<?php } ?> class=" <?php echo pmpro_getClassForField("CardType");?>">
						<?php foreach($pmpro_accepted_credit_cards as $cc) { ?>
							<option value="<?php echo vibe_sanitizer($cc)?>" <?php if($CardType == $cc) { ?>selected="selected"<?php } ?>><?php echo vibe_sanitizer($cc)?></option>
						<?php } ?>												
					</select> 
				</div>
				<?php } ?>
				...
				<div>
					...
					<select id="ExpirationMonth" <?php if($gateway != "stripe") { ?>name="ExpirationMonth"<?php } ?> class=" <?php echo pmpro_getClassForField("ExpirationMonth");?>">
						...
						<?php
							...
							{
						?>
							<option value="<?php echo vibe_sanitizer($i)?>" <?php if($ExpirationYear == $i) { ?>selected="selected"<?php } ?>><?php echo vibe_sanitizer($i)?></option>
						<?php
							}
						?>
					</select> 					
				...
				<?php if($pmpro_show_discount_code) { ?>
				<div>
					<label for="discount_code"><?php _e('Discount Code', 'vibe');?></label>
					...
					<input type="button" id="discount_code_button" name="discount_code_button" value="<?php _e('Apply', 'vibe');?>" />
	...
	<?php					
		...
		<thead>
		<tr>
			<th><?php echo vibe_sanitizer($tospage->post_title)?></th>
		</tr>
	</thead>
		<tbody>
			<tr class="odd">
...
<?php echo wpautop($tospage->post_content)?>
					...
					<input type="checkbox" name="tos" value="1" id="tos" /> <label for="tos"><?php printf(__('I agree to the %s', 'vibe'), $tospage->post_title);?></label>
				</td>
			</tr>
		</tbody>
		</table>
	...
	<div class="pmpro_submit">
		<?php if($pmpro_review) { ?>
			...
			<span id="pmpro_submit_span">
				...
				<input type="submit" class="button primary" value="<?php _e('Complete Payment', 'vibe');?> &raquo;" />
			</span>
		...
		<?php } else { ?>
			...
			<span id="pmpro_paypalexpress_checkout" <?php if(($gateway != "paypalexpress" && $gateway != "paypalstandard") || !$pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				...
				<input type="image" value="<?php _e('Check Out with PayPal', 'vibe');?> &raquo;" src="<?php echo apply_filters("pmpro_paypal_button_image", "https://www.paypal.com/en_US/i/btn/btn_xpressCheckout.gif");?>" />
			</span>
			<?php } ?>
			
			<span id="pmpro_submit_span" <?php if(($gateway == "paypalexpress" || $gateway == "paypalstandard") && $pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<input type="hidden" name="submit-checkout" value="1" />		
				<input type="submit" class="button primary" value="<?php if($pmpro_requirebilling) { if($gateway == "twocheckout") { _e('Submit and Pay with 2CheckOut', 'vibe'); } else { _e('Submit and Check Out', 'vibe'); } } else { _e('Submit and Confirm', 'vibe');}?> &raquo;" />				
			</span>
		<?php } ?>
		
		<span id="pmpro_processing_message" style="visibility: hidden;">
			<?php 
				$processing_message = apply_filters("pmpro_processing_message", __("Processing...", "vibe"));
				echo vibe_sanitizer($processing_message);
			?>					
		</span>
	</div>	
