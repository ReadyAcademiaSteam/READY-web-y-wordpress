...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
			...
			$this->features = array(
	                    		...
	                    		'slider'=>array(
	                    						'label'=>__('[ RECOMMENDED ] Slider','vibe'),
	                    		...
	                    		'elementor'=>array(
	                    						...
	                    						'default'=>1,
	                    						'icon'=>'images/directory.png',
	                    						'description'=> __('New modern page builder for WordPress.','vibe'),
	                    						'verify'=>array('elementor/elementor.php')
                    						),
	                    		'events'=>array(
	                    						'label'=>__('Events','vibe'),
			...
			$this->site_styles = array(
               ...
               'demo11' => array('label'=>'Demo 11','link'=>'https://wplms.io/demos/demo11/'),
               'demo12' => array('label'=>'Demo 12','link'=>'https://wplms.io/demos/demo12/'),
               'demo13' => array('label'=>'Demo 13','link'=>'https://wplms.io/demos/demo13/'),
               'demo14' => array('label'=>'Demo 14','link'=>'https://wplms.io/demos/demo14/'),
               'demo15' => array('label'=>'Demo 15','link'=>'https://wplms.io/demos/demo15/'),
		...
		function envato_start_setup(){
            ...
            <form method="post">
                <p><?php echo esc_html_e( 'Select the features you need for your site. The features here are pre-configured, so the next steps would be based on the selection you make here. Ofcourse these features can be added/removed later on from theme settings as well.','vibe' ); ?></p>
                <hr>
                ...
                <p class="envato-setup-actions step">
                    ...
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step','vibe' ); ?></a>
		...
		public function envato_page_setup(){
			?><h1><?php esc_html_e( 'Set required pages','vibe' ); ?></h1>
	...
	public function setup_wizard_header() {
		...
		public function envato_setup_introduction() {
			...
			if ( isset( $_REQUEST['debug'] ) ) {
				...
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','vibe' ); ?></a>
					</li>
					<li>
						<form method="post">
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel','vibe' ); ?></a>
				</p>
				<?php
			} else {
				...
				<p><?php 
					printf( '<h4>'.esc_html__( 'Welcome to the setup wizard for WPLMS Theme. You\'re using %s theme.','vibe').'</h4><hr>',wp_get_theme());
					printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.','vibe' ), 'WPLMS' ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!','vibe' ); ?></p>
				
				<h2><?php echo esc_html__( 'Configuration Check','vibe'); ?></h2>
				<ul class="config">
				<?php
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins','vibe' ); ?></h1>
			<form method="post">
		...
		private function _content_default_get() {
			...
			$content['slider'] = array(
				...
				'pending'          => esc_html__( 'Pending.' ,'vibe'),
				'installing'       => esc_html__( 'Installing Slider.','vibe' ),
				'success'          => esc_html__( 'Success.','vibe' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content','vibe' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.' ,'vibe'); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. %s Re-installing content from another demo, %s clear cache %s before content re-import.','vibe' ),'<hr>','<a class="clear_imported_posts" data-security="'.wp_create_nonce('wplms_clear_imported_posts').'">','</a>' ); ?></p>
				<?php } ?>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue' ,'vibe'); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step','vibe' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_demo_style() {
			...
			?>
            <h1><?php esc_html_e( 'Site Style','vibe' ); ?></h1>
            <form method="post">
                <p><?php esc_html_e( 'Please choose your site style below.','vibe' ); ?></p>
	                    ...
	                    foreach ( $this->site_styles as $style_name => $style_data ) {
                            ...
                            <li<?php echo vibe_sanitizer($style_name == $current_style ? ' class="current" ' : ''); ?>>
                                <a href="#" class="sitestyle" data-style="<?php echo esc_attr( $style_name ); ?>"><img
                                            src="<?php echo esc_url(get_template_directory_uri() .'/setup/installer/images/'.$style_name.'.jpg');?>"></a><a href="<?php echo vibe_sanitizer($style_data['link'],'url'); ?>" target="_blank" class="link"></a>
                            </li>
	                    <?php } ?>
                    </ul>
                </div>
			...
			<form method="post">
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload" data-title="Upload a logo" data-text="select a logo" data-target="#current-logo img" data-save="#logo_url"><?php esc_html_e( 'Upload New Logo' ,'vibe'); ?></a>
						</td>
					</tr>
					<tr>
						<td>
				...
				<div class="envato-setup-next-steps-first">
					...
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             ...
						                             target="_blank"><?php esc_html_e( 'Follow @vibethemes on Twitter' ,'vibe'); ?></a>
						</li>
